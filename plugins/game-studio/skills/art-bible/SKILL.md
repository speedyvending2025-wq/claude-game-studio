...
See `references/director-gates.md` for the full check pattern.
...
Read `.game-studio/project-profile.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `.game-studio/project-profile.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`references/director-gates.md`).
...
- Does `.game-studio/project-profile.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
